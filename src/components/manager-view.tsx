...
import { CheckCircle, ChefHat, Utensils, Package, Clock, ShieldAlert, XCircle, FileText, Printer, BarChart, HardHat, PlusCircle } from 'lucide-react';
...
interface ManagerViewProps {
  ...
  onOpenOrderForm: (order: Order | null) => void;
  onPrintBill: (bill: Bill | null) => void;
...
const SectionHeader = ({ title, count }: { title: string, count: number }) => (
    <div className="flex items-center gap-2">
        ...
        {count > 0 && <Badge className="h-6 w-6 flex items-center justify-center rounded-full bg-orange-500 text-white hover:bg-orange-500/90">{count}</Badge>}
    </div>
);
...
export default function ManagerView({
  ...
  const getWaiterName = (waiterId: string) => {
    if (waiterId === 'manager') return 'Manager';
  ...
  const { tablesToBill, unpaidBills } = useMemo(() => {
    ...
    const ordersByTable = orders.reduce((acc, order) => {
        if (!['billed', 'cancelled'].includes(order.status)) { 
            if (order.tableNumber && !acc[order.tableNumber]) {
    ...
    switch (order.status) {
            ...
            return (
                <div className='flex items-center gap-2 w-full'>
                    <Button
                      ...
                      onClick={() => setConfirmation({ orderId: order.id, status: 'approved', message: `This will send the order for ${order.orderType === 'pickup' ? `Pickup #${order.id.slice(-4)}` : `Table ${order.tableNumber}`} to the kitchen.` })}
                    >
        ...
        case 'approved':
             return (
                <div className='w-full flex flex-col gap-2'>
                    <Button
                        className="w-full"
                        onClick={() => setConfirmation({ orderId: order.id, status: 'prepared', message: `This will mark the order for ${order.orderType === 'pickup' ? `Pickup #${order.id.slice(-4)}` : `Table ${order.tableNumber}`} as prepared.` })}
                    >
                        <ChefHat className="mr-2 h-4 w-4" /> Mark as Prepared
                    </Button>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                           <Button variant="ghost" className='w-full text-xs h-auto py-1'>
                                More Actions
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
        ...
        case 'prepared':
             if (order.orderType === 'pickup') {
                 return <span className='text-sm text-muted-foreground'>Order is ready for pickup.</span>
             }
             return (
                <Button className="w-full" onClick={() => setConfirmation({ orderId: order.id, status: 'served', message: `This will mark the order for Table ${order.tableNumber} as served.` })}>
        ...
        content: (
            <div className="space-y-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h3 className="text-xl font-headline font-semibold">Today's Live Stats</h3>
                        <p className="text-sm text-muted-foreground">A real-time overview of today's performance.</p>
                    </div>
                    <Button onClick={() => onOpenOrderForm(null)}>
                ...
                <div>
                    <div className="mb-4">
                        <SectionHeader title="Pending Approval" count={pendingOrders.length} />
                    </div>
                ...
                <div>
                    <div className="mb-4">
                        <SectionHeader title="In the Kitchen" count={approvedOrders.length} />
                    </div>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <div>
                     <div className="mb-4">
                        <SectionHeader title="Ready to Serve / Pickup" count={preparedOrders.length} />
                    </div>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        ...
                        preparedOrders.map(order => (
                            ...
                            <Card className="col-span-full border-dashed">
                                <CardHeader className="text-center">
                                    ...
                                    <CardDescription>No orders are currently waiting to be served or picked up.</CardDescription>
                                </CardHeader>
                            </Card>
                        )}
                    </div>
                     ...
                     paidBillsWithDetails.map(({ bill, items, waiterId }) => (
                        <OrderCard
                            ...
                            order={{ // We adapt the bill to look like an order for the card
                                ...
                                waiterId: waiterId,
                                orderType: bill.tableNumber ? 'dine-in' : 'pickup',
                            }}
